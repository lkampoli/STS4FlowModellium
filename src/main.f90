...
!call PopulateEnergy() ! moved to constants
...
! Here, I just call `compute_rhs` to 
! compute the RHS only once instead of 
! solving the full AX=B system at each
! step. This is typically what we would
! call when interfacing with CFD solvers.
! Note: that depending on the dimensions
!       of the Y0_bar input vector
 block
   real(WP) :: RHS(neq)
   write(*,*) "Computing the RHS once ..."
   call compute_rhs(neq, Y0_bar, RHS)
   write(*,*) "Y0_bar = ", Y0_bar
   write(*,*) "RHS = ", RHS
   write(*,*) "Computing the RHS once ... OK!"
 end block
...
! vibrational energy (ground level) 
